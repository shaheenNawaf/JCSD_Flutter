...
class _AddProdDefModalState extends ConsumerState<AddProdDefModal> {
  ...
  Future<void> submitForm() async {
    ...
    if(!formKey.currentState!.validate()){
      ToastManager().showToast(context, 'Please fix errors in the form', Colors.orange);
    } 
    ...
    try{
      await ref.read(productDefinitionNotifierProvider(widget.isVisible).notifier).addProductDefinition(newProdDef);

      print('Trying to add a new Product Definition');
      await Future.delayed(const Duration(seconds: 1));

      ToastManager().showToast(context, 'Product Definition added successfully!', Colors.green);
      Navigator.pop(context);
    }catch(err, st){
      print("Error adding Product Definition: $err\n$st");
      ToastManager().showToast(context, 'Failed to add Product Definition: ${err.toString()}', Colors.red);
    }finally {
      if (mounted) {
  ...
  Widget build(BuildContext context){
    ...
    double containerWidth = screenWidth > 700 ? 650 : screenWidth * 0.9; // Dynamic width

    return Dialog(
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
      insetPadding: EdgeInsets.symmetric(horizontal: screenWidth > 600 ? 50.0 : 16.0),
      child: SizedBox( // Use SizedBox for better sizing control
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                ...
                child: const Center(
                  child: Text('Add New Product Definition', style: TextStyle(fontFamily: 'NunitoSans', fontWeight: FontWeight.bold, fontSize: 20, color: Colors.white)),
                ),
              ),
                     ...
                     children: [
                        ...
                        Expanded(
                           child: Column(
                              ...
                              children: [
                                 _buildTextField( // Reusable text field widget
                                    label: 'Product Name',
                        ...
                        Expanded(
                           child: Column(
                              ...
                              children: [
                                 ...
                                 const SizedBox(height: 15),
                                 _buildManufacturerDropdown(ref), // Manufacturer dropdown
                                 const SizedBox(height: 15),
                                 _buildTextField( // Reusable text field widget
                                    label: 'MSRP (PHP)',
                                    ...
                                    isRequired: true,
                                    validator: (value) { // Price validation
                                      if (value == null || value.isEmpty) return 'MSRP is required';
              ...
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    Expanded( // Makes buttons flexible
                      child: TextButton(
                        onPressed: isSaving ? null : () => Navigator.pop(context), // Disable if saving
                    ...
                    Expanded( // Makes buttons flexible
                      child: ElevatedButton.icon(
                        icon: isSaving ? Container() : const Icon(Icons.add_circle_outline, size: 18), // Show icon only when not saving
                        label: isSaving
                            ? const SizedBox(height: 18, width: 18, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) // Loading indicator
                            : const Text('Add Product'),
                        onPressed: isSaving ? null : submitForm, // Disable if saving
  ...
  Widget _buildTextField({
    ...
    return Column( // Structure for label and field
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(children: [ Text(label), if (isRequired) const Text(' *', style: TextStyle(color: Colors.red)) ]), // Label with optional required marker
        const SizedBox(height: 5),
        TextFormField( // The actual input field
          ...
          validator: validator ?? (value) { // Default required field validation
  ...
  Widget _buildItemTypeDropdown(WidgetRef ref) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Row(children: [ Text('Item Type'), Text(' *', style: TextStyle(color: Colors.red)) ]), 
        const SizedBox(height: 5),
        itemTypesAsync.when( 
          data: (types) => DropdownButtonFormField<int>( 
            ...
            isExpanded: true, 
            items: types.map((type) => DropdownMenuItem<int>( // Create dropdown items from fetched data
                  value: type.itemTypeID,
                  child: Text(type.itemType, style: const TextStyle(fontSize: 12)),
                )).toList(),
            onChanged: (value) => setState(() => selectedItemTypeID = value), // Update state on selection
            validator: (value) => value == null ? 'Item Type is required' : null, // Validation
  ...
  Widget _buildManufacturerDropdown(WidgetRef ref) {
    ...
    final manufacturersAsync = ref.watch(activeManufacturersForDropdownProvider); 
    return Column( 
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Row(children: [ Text('Manufacturer'), Text(' *', style: TextStyle(color: Colors.red)) ]),
        const SizedBox(height: 5),
        manufacturersAsync.when( // Handle loading/error/data states
          data: (manufacturers) => DropdownButtonFormField<String>( 
            value: selectedManufacturerName, // Currently selected name
            hint: const Text('Select manufacturer...', style: TextStyle(fontSize: 12, color: Colors.grey)),
            decoration: const InputDecoration(border: OutlineInputBorder(), contentPadding: EdgeInsets.symmetric(vertical: 10.0, horizontal: 12.0)), 
            isExpanded: true,
            items: manufacturers.map((mfg) => DropdownMenuItem<String>( 
                  value: mfg.manufacturerName, // Value is the name string
                  child: Text(mfg.manufacturerName, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 12)),
                )).toList(),
            onChanged: (value) => setState(() => selectedManufacturerName = value), 
            validator: (value) => value == null ? 'Manufacturer is required' : null, // Validation
            autovalidateMode: AutovalidateMode.onUserInteraction,
          ),
          loading: () => const Center(child: SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2))),
          error: (err, stack) => Text('Error loading manufacturers: $err', style: const TextStyle(color: Colors.red, fontSize: 12)), 
        ),
      ],
    );
  }
