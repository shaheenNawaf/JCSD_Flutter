...
class EditProductDefinitionModal extends ConsumerStatefulWidget {
  final ProductDefinitionData productDefinition; // Pass the existing data to edit
  final bool isVisibleContext; // Context from which it was called (true = active list)
  ...
  @override
  ConsumerState<EditProductDefinitionModal> createState() => _EditProductDefinitionModalState();
}

class _EditProductDefinitionModalState extends ConsumerState<EditProductDefinitionModal> {
  ...
  void initState() {
    ...
    _descriptionController = TextEditingController(text: pd.prodDefDescription ?? '');
    _msrpController = TextEditingController(text: pd.prodDefMSRP?.toStringAsFixed(2) ?? '');
  ...
  Future<void> _submitForm() async {
    ...
    final updatedProdDef = widget.productDefinition.copyWith(
      ...
      prodDefMSRP: double.tryParse(_msrpController.text.trim()) ?? widget.productDefinition.prodDefMSRP, // Update price safely
    );
    ...
    try {
      ...
      ToastManager().showToast(context, 'Product Definition updated successfully!', Colors.green);
      Navigator.pop(context); // Close dialog on success

    } catch (e, st) {
      print("Error updating Product Definition: $e\n$st");
      ToastManager().showToast(context, 'Failed to update Product Definition: ${e.toString()}', Colors.red);
    } finally {
      if (mounted) {
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                 ...
                 child: Center(
                  child: Text('Edit Product Definition (ID: ${widget.productDefinition.prodDefID}...)',
              ...
              Padding(
                 ...
                 child: Row(
                    ...
                    children: [
                       Expanded(
                         child: TextButton( // Cancel button
                       ...
                       Expanded(
                         child: ElevatedButton.icon( // Save Changes button
                           icon: isSaving ? Container() : const Icon(Icons.save, size: 18),
                           label: isSaving
                              ? const SizedBox(height: 18, width: 18, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
                              : const Text('Save Changes'),
                           onPressed: isSaving ? null : _submitForm,
                           style: ElevatedButton.styleFrom(
                              backgroundColor: const Color(0xFF00AEEF), // Consistent color
      ...
      children: [
        ...
        itemTypesAsync.when(
          data: (types) => DropdownButtonFormField<int>(
            ...
            isExpanded: true,
            items: types.map((type) => DropdownMenuItem<int>(
                  value: type.itemTypeID,
                  child: Text(type.itemType, style: const TextStyle(fontSize: 12)),
                )).toList(),
            onChanged: (value) => setState(() => selectedItemTypeID = value), // Update state
  ...
  Widget _buildManufacturerDropdown(WidgetRef ref) {
    final manufacturersAsync = ref.watch(activeManufacturersForDropdownProvider); // Fetch active manufacturers
    ...
    return Column(
      ...
      children: [
        ...
        manufacturersAsync.when(
          data: (manufacturers) => DropdownButtonFormField<String>( // Use String for value now
            ...
            onChanged: (value) => setState(() => selectedManufacturerName = value), // Update state with name
