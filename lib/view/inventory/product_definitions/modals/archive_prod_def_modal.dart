// lib/view/inventory/modals/archive_prod_def_modal.dart
...
class ArchiveProductDefinitionModal extends ConsumerStatefulWidget {
  final String prodDefID;
  final String prodDefName;
  final bool isVisibleContext;

  const ArchiveProductDefinitionModal({
    super.key,
  ...
  @override
  ConsumerState<ArchiveProductDefinitionModal> createState() => _ArchiveProductDefinitionModalState();
}

class _ArchiveProductDefinitionModalState extends ConsumerState<ArchiveProductDefinitionModal> {
  ...
  Future<void> _archiveItem() async {
    ...
    try {
      await ref.read(productDefinitionNotifierProvider(widget.isVisibleContext).notifier).updateProductDefinitionVisibility(widget.prodDefID, false);

      ToastManager().showToast(context, 'Product "${widget.prodDefName}" archived.', Colors.green);
      Navigator.pop(context);

    } catch (e, st) {
      print("Error archiving Product Definition ${widget.prodDefID}: $e\n$st");
      ToastManager().showToast(context, 'Failed to archive: ${e.toString()}', Colors.red);
       if (mounted) {
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: const Center(
                child: Text('Confirm Archive', style: TextStyle(fontFamily: 'NunitoSans', fontWeight: FontWeight.bold, fontSize: 20, color: Colors.white)),
              ),
            ),
            Padding(
                ...
                children: [
                  ...
                  Expanded(
                    child: ElevatedButton.icon(
                      icon: _isArchiving ? Container() : const Icon(Icons.archive, size: 18),
                      label: _isArchiving
