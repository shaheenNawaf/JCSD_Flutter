...
abstract class BookingRepository {
  ...
  Future<List<Booking>> getPotentiallyOverlappingBookings(
    String customerUserId,
    DateTime checkStartTime, // proposedStartTime - buffer
    DateTime checkEndTime, // proposedEndTime + buffer
    List<BookingStatus> statuses,
  ) async {
    try {
      final statusStrings = statuses
          .map((s) => s.name)
          .toList(); // Assuming BookingStatus has a .name for the enum string

      final response = await supabaseDB
          .from('bookings')
          .select('*, booking_services(*, jcsd_services(*))')
          .eq('customer_user_id', customerUserId)
          .or('status.eq.${statusStrings[0]},status.eq.${statusStrings[1]}')
          .lt('scheduled_start_time', checkEndTime.toIso8601String())
          .gt('scheduled_end_time', checkStartTime.toIso8601String());
      ...
      print("BookingRepository Error fetching overlapping bookings: $e");
      throw Exception("Failed to query for overlapping bookings: $e");
    }
  }
