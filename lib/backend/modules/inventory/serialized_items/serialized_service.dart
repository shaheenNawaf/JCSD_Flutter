...
class SerialitemService {
  ...
  Future<int> getTotalSerializedItemCount({
    ...
    try {
      ...
      if (searchQuery != null && searchQuery.isNotEmpty) {
        final searchTerm = '%$searchQuery%';
        fetchSerialsCount = fetchSerialsCount
            .or('serialNumber.ilike.$searchTerm,notes.ilike.$searchTerm');
      }
  ...
  Future<SerializedItem?> getSerializedItemBySerial(String serialNumber) async {
    try {
      ...
      final fetchSerialDetails = await supabaseDB
          .from('item_serials')
          .select(selectQuery)
  ...
  Future<SerializedItem> addSerializedItem(SerializedItem newItem) async {
    try {
      ...
      final serialResult = await supabaseDB
          .from('item_serials')
          .insert(newSerialItem)
  ...
  Future<SerializedItem> updateSerializedItem(
    ...
    try {
      final dataToUpdate = {
        ...
        'bookingID': updatedItem.bookingID,
      ...
      final serialUpdateResult = await supabaseDB
          .from('item_serials')
          .update(dataToUpdate)
          .eq('serialNumber', updatedItem.serialNumber)
          .select(returnSelectQuery)
          .single();
