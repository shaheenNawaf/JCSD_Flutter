...
final productDefinitionNotifierProvider = AutoDisposeAsyncNotifierProviderFamily<
    ProductDefinitionNotifier, ProductDefinitionState, bool>(
...
class ProductDefinitionNotifier
  ...
  late bool _isVisible; // Stores visibility context (true=active, false=archived)
  ...
  Future<ProductDefinitionState> build(bool arg) async {
    _isVisible = arg; // Stores the family parameter (true for active, false for archived)
    print('ProductDefinitionNotifier build: isVisible = $_isVisible'); // Debug log
  ...
  Future<void> addProductDefinition(ProductDefinitionData newProductDefinition) async {
    state = const AsyncValue.loading(); // Set global loading (optional)
    
    // Uses guard for async operation and error handling
    state = await AsyncValue.guard(() async {
      await _service.addProductDefinition(newProductDefinition); // Calls service
